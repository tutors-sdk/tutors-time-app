<script lang="ts">
  ...
  $effect(() => {
    ...
    const api = createGrid<PivotedRow>(container, {
      columnDefs: model.week.columnDefs,
      rowData: model.week.rows,
  ...
  $effect(() => {
    const api = gridApi;
    if (api) {
      api.setGridOption("columnDefs", model.week.columnDefs);
      api.setGridOption("rowData", model.week.rows);
...
{:else}
  <div class="ag-theme-quartz grid-fill-container h-full min-h-0" role="grid" aria-label="Course usage by student and week">
